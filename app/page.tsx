...
type Budget = { id: string; total_budget: number; month: string };
type Transaction = {
  id: string;
  merchant: string;
  amount: number;
...
export default function Home() {
  ...
  async function loadBudget() {
    ...
    if (data.budget) setSafeToSpend(Number(data.budget.total_budget));
  }
  ...
  async function handleAddExpense(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/expense", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ merchant, amount: amt }),
      });
      const data = await res.json();
      ...
      if (data.flag && data.flag.flagged) {
        setAlerts((prev) => [
          { id: data.transaction.id, flag: data.flag, merchant },
          ...prev,
        ]);
      }
      setMerchant("");
        ...
        <>
          ...
          {alerts.length > 0 && (
            <div className="card">
              <h2>Nudges</h2>
              {alerts.map((a) => (
                <div key={a.id} className={`alert ${a.flag.severity}`}>
                  <div className="alert-severity">{a.flag.severity}</div>
                  <div>
            ...
            {transactions.length === 0 ? (
              ...
              transactions.map((t) => (
                <div className="txn-row" key={t.id}>
                  ...
                  <span className="txn-amount">₹{Number(t.amount).toFixed(0)}</span>
                </div>
              ))
            )}
          </div>
          ...
          <div className="card">
            ...
            {summary ? (
              ...
              <button className="btn" onClick={handleGetSummary} disabled={summaryLoading}>
                {summaryLoading ? "Writing your recap…" : "Generate wrap-up"}
              </button>
            )}
          </div>
          ...
          {showAudit && (
            <div className="card" style={{ marginTop: 14 }}>
              ...
              <p style={{ color: "var(--text-dim)", fontSize: 13 }}>
                Every AI decision — categorization, budget nudges, and this
                summary — is logged in Supabase&apos;s <code>ai_logs</code>{" "}
                table with the exact (privacy-sanitized) input it saw and
                whether a fallback rule fired instead. Query that table
                directly to inspect the full trail for your demo.
              </p>
            </div>
          )}
        </>
